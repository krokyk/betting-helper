 ...
 * @author Kroky
 */
public class ScoresProFixturesHTMLParser extends AbstractParser {

    private static final Logger LOG = org.apache.log4j.Logger.getLogger(ScoresProFixturesHTMLParser.class);
    /*
     ...
     */
    private static final Pattern PATTERN_DATE = Pattern.compile("class=\"kick_t_dt\"[^>]*>([^<]+)<.*class=\"kick_t_ko\"[^>]*>([^<]+)<.*<a[^>]+>([^<]+)<.*<a[^>]+>([^<]+)<");

    private static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("dd.MM.yy HH:mm");

    public ScoresProFixturesHTMLParser() {
        super("www.scorespro.com 02 Fixtures (HTML)");
        LOG.debug("Using pattern: " + PATTERN_DATE.pattern());
    }
    ...
    public Set<ParseResult> parse(String text) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (matcher.find()) {
                    dateStr = matcher.group(1).trim() + " " + matcher.group(2).trim();
