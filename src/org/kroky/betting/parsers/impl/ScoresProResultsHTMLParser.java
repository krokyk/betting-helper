 ...
 * @author Kroky
 */
public class ScoresProResultsHTMLParser extends AbstractParser {
    ...
    private static final Pattern PATTERN = Pattern.compile("class=\"kick_t_dt\"[^>]*>([^<]+)<.*class=\"kick_t_ko\"[^>]*>([^<]+)<.*<a[^>]+>([^<]+)<.*<a[^>]+>([^<]+)<.*<a[^>]+>([^<]+)<");
    ...
    private static final Pattern ODDS_EXTRACT_PATTERN = Pattern.compile("data-odd=\"\\d+.\\d+\".+data-odd=\"(\\d+.\\d+)\".+data-odd=\"\\d+.\\d+\".+(\\d+\\.\\d+)"); //no odds on this website

    public ScoresProResultsHTMLParser() {
        super("www.scorespro.com 01 Results (HTML)");
    ...
    public Set<ParseResult> parse(String text) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (matcher.find()) {
                    String homeTeamName = matcher.group(3).trim();
                    ...
                    String[] goals = matcher.group(4).split("\\s*-\\s*");
                    ...
                    String dateStr = matcher.group(1).trim() + " " + matcher.group(2).trim();
                    Timestamp date;
                    try {
